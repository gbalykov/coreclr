...
PTR_MethodDesc MethodImpl::FindMethodDesc(DWORD slot, PTR_MethodDesc defaultReturn)
{
    ...
    DPTR(RelativePointer<PTR_MethodDesc>) pRelPtrForSlot = GetImplementedMDsNonNull();
    PTR_MethodDesc result = pRelPtrForSlot[slotIndex].GetValueMaybeNull(dac_cast<TADDR>(pRelPtrForSlot) + slotIndex * sizeof(RelativePointer<MethodDesc *>)); // The method descs are not offset by one
...
void MethodImpl::SetData(DWORD* slots, RelativePointer<MethodDesc*>* md)
{
    ...
    PTR_DWORD pdwSize = pdwSlots.GetValue();
    ...
    {
        pImplMD[i].SetValueMaybeNull(md[i].GetValueMaybeNull());
    }
}
...
MethodImpl::EnumMemoryRegions(CLRDataEnumMemoryFlags flags)
{
    ...
    if (GetSlots().IsValid() && GetSize())
    {
        ULONG32 numSlots = GetSize();
        DacEnumMemoryRegion(dac_cast<TADDR>(GetSlots()),
                            (numSlots + 1) * sizeof(DWORD));

        if (GetImplementedMDs().IsValid())
        {
            DacEnumMemoryRegion(dac_cast<TADDR>(GetImplementedMDsNonNull()),
                                numSlots * sizeof(RelativePointer<MethodDesc *>));
            for (DWORD i = 0; i < numSlots; i++)
            {
                DPTR(RelativePointer<PTR_MethodDesc>) pRelPtr = GetImplementedMDsNonNull();
                PTR_MethodDesc methodDesc = pRelPtr[i].GetValueMaybeNull(dac_cast<TADDR>(pRelPtr) + i * sizeof(RelativePointer<MethodDesc *>));
