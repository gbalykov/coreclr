    ...
    AllocMemTracker *pamTracker)
{
    ...
    pMT->SetPerInstInfo ((DPTR(MethodTable::PerInstInfoElem_t)) (pMemory + cbMT + cbOptional + cbIMap + sizeof(GenericsDictInfo)));
    ...
    (dac_cast<DPTR(MethodTable::PerInstInfoElem_t)>(pMT->GetPerInstInfo() + (pOldMT->GetNumDicts()-1)))->SetValueMaybeNull(pDict);
