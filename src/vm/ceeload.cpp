...
void ModuleCtorInfo::Save(DataImage *image, CorProfileData *profileData)
{
    ...
    for (i = 0; i < numElements; i++)
    {
        ...
        if (hot)
        {
            ...
            ppMT[i].SetValueMaybeNull(ppMT[numElementsHot].GetValueMaybeNull());
    ...
    {
        hashArray[i] = GenerateHash(ppMT[i].GetValueMaybeNull(), HOT);
    }
    for (i = numElementsHot; i < numElements; i++)
    {
        hashArray[i] = GenerateHash(ppMT[i].GetValueMaybeNull(), COLD);
    }
