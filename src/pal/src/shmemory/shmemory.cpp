...
static SHM_FIRST_HEADER *shm_header;
...
BOOL SHMInitialize(void)
{
    InternalInitializeCriticalSection(&shm_critsec);
        ...
        shm_header = (SHM_FIRST_HEADER *) malloc (sizeof (SHM_FIRST_HEADER));
...
SHMPTR SHMalloc(size_t size)
{
    ...
    shmptr = (SHMPTR) malloc (size);

    SHMRelease();

    TRACE("Allocation successful. Returning 0x%08x\n", shmptr);
