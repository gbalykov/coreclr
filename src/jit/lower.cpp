...
GenTree* Lowering::LowerVirtualVtableCall(GenTreeCall* call)
{
    ...
    unsigned isRelative;
    comp->info.compCompHnd->getMethodVTableOffset(call->gtCallMethHnd,
    ...
    if (vtabOffsOfIndirection != CORINFO_VIRTUALCALL_NO_CHUNK)
    {
        if (isRelative)
        {
            result = comp->gtNewOperNode(GT_ADD, TYP_I_IMPL, result, comp->gtNewIconNode(vtabOffsOfIndirection, TYP_I_IMPL));
            result1 = comp->gtNewOperNode(GT_ADD, TYP_I_IMPL, result1, comp->gtNewIconNode(vtabOffsOfIndirection, TYP_I_IMPL));
