                ...
                // stub dispatching is off or this is not a virtual call (could be a tailcall)
                {
                    ...
                    if (vtabOffsOfIndirection != CORINFO_VIRTUALCALL_NO_CHUNK)
                    {
                        if (isRelative)
                        {
                            ...
                            // MOV vptrReg1, [REG_CALL_IND_SCRATCH + vtabOffsOfIndirection]
                            getEmitter()->emitIns_R_R_I(ins_Load(TYP_I_IMPL), EA_PTRSIZE, vptrReg1, vptrReg,
                                                        vtabOffsOfIndirection);

                            if (unsigned_abs(offset) <= EMIT_ARM_MAX_IMM_ADD)
                            {
                                // ADD vptrReg, vtabOffsOfIndirection + vtabOffsAfterIndirection
                                getEmitter()->emitIns_R_I(INS_add, EA_PTRSIZE, vptrReg, offset);
                            }
                            else
                            {
                                // ADD vptrReg1, vptrReg1, vptrReg
                                getEmitter()->emitIns_R_R_I(INS_add, EA_PTRSIZE, vptrReg1, vptrReg1, vptrReg);

                                // MOVW, MOVT
                                instGen_Set_Reg_To_Imm(EA_PTRSIZE, vptrReg, offset);
                            }
#else
                            _ASSERTE(false);
#endif
                    ...
                    else
                    {
                        ...
                        if (isRelative)
                        {
                            getEmitter()->emitIns_R_ARR(ins_Load(TYP_I_IMPL), EA_PTRSIZE, vptrReg, vptrReg, vptrReg1,
                                                        0);
                        }
                        else
                        {
