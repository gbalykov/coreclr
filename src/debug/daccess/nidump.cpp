...
void NativeImageDumper::DumpMethodDesc( PTR_MethodDesc md, PTR_Module module )
{
    ...
    if (md->HasMethodImplSlot())
    {
        ...
        _ASSERTE(impl->pImplementedMD.GetValueMaybeNull() == NULL
                 || isInRange(PTR_TO_TADDR(impl->pImplementedMD.GetValueMaybeNull())));
        if ((impl->pImplementedMD.GetValueMaybeNull() != NULL) &&
            isInRange(PTR_TO_TADDR(impl->pImplementedMD.GetValue())))
        {
            DisplayWriteFieldAddress( pImplementedMD,
                                      DataPtrToDisplay(dac_cast<TADDR>(impl->pImplementedMD.GetValue())),
        ...
        else
        {
            DisplayWriteFieldPointer( pImplementedMD,
                                      DataPtrToDisplay(dac_cast<TADDR>(impl->pImplementedMD.GetValueMaybeNull())),
                                      MethodImpl, METHODDESCS );
        }
    ...
    if (md->HasStoredSig())
    {
        ...
        if( isInRange(ssmd->m_pSig.GetValueMaybeNull()) )
        {
            DisplayWriteFieldAddress(m_pSig, DataPtrToDisplay(ssmd->m_pSig.GetValueMaybeNull()),
                                     ssmd->m_cSig, StoredSigMethodDesc,
                                     METHODDESCS);
        }
        else
        {
            DisplayWriteFieldPointer(m_pSig, DataPtrToDisplay(ssmd->m_pSig.GetValueMaybeNull()),
                                     StoredSigMethodDesc, METHODDESCS);

        }
        CoverageRead(TO_TADDR(ssmd->m_pSig.GetValueMaybeNull()), ssmd->m_cSig);
    ...
    if( mc == mcDynamic )
    {
        ...
        WriteFieldStr( m_pszMethodName, PTR_BYTE(dmd->m_pszMethodName.GetValueMaybeNull()),
                       DynamicMethodDesc, METHODDESCS );
        if( !CHECK_OPT(METHODDESCS) )
            CoverageReadString( PTR_TO_TADDR(dmd->m_pszMethodName.GetValueMaybeNull()) );
    ...
    if( mc == mcNDirect )
    {
        ...
        WriteFieldStr( m_pszEntrypointName,
                       PTR_BYTE(dac_cast<TADDR>(nd->m_pszEntrypointName.GetValueMaybeNull())),
                       NDirectMethodDesc::temp1, METHODDESCS );
        if( !CHECK_OPT(METHODDESCS) )
            CoverageReadString(dac_cast<TADDR>(nd->m_pszEntrypointName.GetValueMaybeNull()));
        if (md->IsQCall())
        {
            DisplayWriteFieldInt( m_dwECallID,
                                  nd->m_dwECallID,
        ...
        else
        {
            WriteFieldStr( m_pszLibName,
                           PTR_BYTE(dac_cast<TADDR>(nd->m_pszLibName.GetValueMaybeNull())),
                           NDirectMethodDesc::temp1, METHODDESCS );
        }
        if( !CHECK_OPT(METHODDESCS) )
            CoverageReadString( dac_cast<TADDR>(nd->m_pszLibName.GetValueMaybeNull()) );

        PTR_NDirectWriteableData wnd( nd->m_pWriteableData.GetValueMaybeNull() );
...
#ifdef HAS_NDIRECT_IMPORT_PRECODE
        PTR_NDirectImportThunkGlue glue(nd->m_pImportThunkGlue.GetValue());
#else
    ...
    if( mc == mcInstantiated )
    {
        ...
        {
            PTR_DictionaryLayout layout(dac_cast<TADDR>(imd->m_pDictLayout.GetValueMaybeNull()));
            IF_OPT(METHODDESCS)
            {
        ...
        {
            PTR_MethodDesc wimd(imd->m_pWrappedMethodDesc.GetValueMaybeNull());
        ...
        {
            _ASSERTE(imd->m_pDictLayout.GetValueMaybeNull() == NULL);
        ...
        PTR_Dictionary inst(imd->m_pPerInstInfo.GetValueMaybeNull());
        unsigned dictSize;
        if( kind == InstantiatedMethodDesc::SharedMethodInstantiation )
        {
        ...
        else if( kind == InstantiatedMethodDesc::WrapperStubWithInstantiations )
        {
            PTR_InstantiatedMethodDesc wrapped =
                PTR_InstantiatedMethodDesc(imd->m_pWrappedMethodDesc.GetValueMaybeNull());
            ...
            else
            {
                PTR_DictionaryLayout layout(wrapped->IsSharedByGenericMethodInstantiations()
                                            ? dac_cast<TADDR>(wrapped->m_pDictLayout.GetValueMaybeNull()) : NULL );
