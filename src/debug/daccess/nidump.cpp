...
void NativeImageDumper::DumpMethodDesc( PTR_MethodDesc md, PTR_Module module )
{
    ...
    if (md->HasMethodImplSlot())
    {
        ...
        _ASSERTE(impl->GetImpMDs() == NULL
                 || isInRange(PTR_TO_TADDR(impl->GetImpMDs())));
        if ((impl->GetImpMDs() != NULL) &&
...
#ifdef HAS_NDIRECT_IMPORT_PRECODE
        PTR_NDirectImportThunkGlue glue(ndmd->GetImportThunkGlue());
#else
    ...
    if( mc == mcInstantiated )
    {
        ...
        {
            PTR_DictionaryLayout layout(dac_cast<TADDR>(imd->GetDictLayout()));
            IF_OPT(METHODDESCS)
            {
        ...
        {
            _ASSERTE(imd->GetDictLayout() == NULL);
        ...
        else if( kind == InstantiatedMethodDesc::WrapperStubWithInstantiations )
        {
            ...
            else
            {
                PTR_DictionaryLayout layout(wrapped->IsSharedByGenericMethodInstantiations()
                                            ? dac_cast<TADDR>(wrapped->GetDictLayout()) : NULL );
